    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('type', function ($query) {
                switch ($query->product_type) {
                    case 'new_arriavle':
                        return '<i class="badge bg-success">New Arriavle</i>';
                        break;
                    case 'featured_product':
                        return '<i class="badge bg-info">Featured Product</i>';
                        break;
                    case 'top_product':
                        return '<i class="badge bg-warning">Top Product</i>';
                        break;
                    case 'best_product':
        ...
        ->addColumn('approved',function($query){
            if($query->is_approved==1){
                return '<i class="badge bg-success">Apporved</i>';
          ...
          ->addColumn('action', function($query){
            ...
            $moreBtn = '<div class="btn-group dropstart" style="margin-left:3px">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item has-icon" href="' . route('vendor.products-image-gallary.index', ['product' => $query->id]) . '"> Image Gallery</a></li>
            ...
            ->rawColumns(['thumb_image', 'status','type', 'action','approved'])
            ->setRowId('id');
    }
