...
class VendorProductVariantController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $variant->save();
        return redirect()->route('vendor.products-variant.index', ['product' => $request->product])->with('success', 'Variant Create Succesfully!!');
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $variant->status = $request->status;
        $variant->save();
        return redirect()->route('vendor.products-variant.index', ['product' => $variant->product_id])->with('success', 'Variant Update  Succesfully!!');
    }
    ...
    public function destroy(string $id)
    {
        ...
        if ($variantItemCheck > 0) {
            return response(['status' => 'error', 'message' => 'This variant contain variant items']);
        }
        $variant->delete();
